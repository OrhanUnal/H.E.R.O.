...
var r_ijk: Vector3
var v_ijk: Vector3
var colorOfOrbit = Color(randf_range(0,1),randf_range(0,1),randf_range(0,1))
var impact: bool = false
var impactVelocity = 2
var dislocation = Vector2(0,0)
var orbitChanged = false
var newOrbitCounter = 0
...
func _process(delta: float) -> void:
	if drawTime < p_yr * 365.25 * 86400.0:
		...
		drawOrbit(drawTime, Color(0.994, 0.0, 0.264, 1.0))
		if drawTime >= p_yr * 365.25 * 86400:
			emit_signal("drawFinished")
	sim_time += delta / pow(10, -6)
	if orbitChanged:
		global_position.x = lerpf(global_position.x, update_orbit(sim_time).x * pow(10,-6), 0.3)
		global_position.y = lerpf(global_position.y, update_orbit(sim_time).y * pow(10,-6), 0.3)
	else:
		global_position = update_orbit(sim_time) * pow(10, -6)
	if orbitChanged and newOrbitCounter < 100:
		drawTime += p_yr * 365.25 * 86400.0/100
		drawOrbit(drawTime, Color(0.988, 0.888, 0.296, 1.0))
		newOrbitCounter += 1
	if newOrbitCounter >= 100:
		orbitChanged = false
		newOrbitCounter = 0
		emit_signal("drawFinished")

func drawOrbit(delta, color: Color):
	var M = n * delta
	M = ang(M)
	var E = keplerE(M, e)
	...
	orbitIndicator.get_node_or_null("Sprite2D").modulate = color
